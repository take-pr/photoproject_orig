...
# accounts2アプリのmodelsモジュールからCustom2Userをインポート
from accounts2.models import Custom2User
...
class PhotoPost(models.Model):
    ...
    def search_photos(keyword):
        '''指定されたキーワードで投稿記事を検索する

        Args:
            keyword(str): 検索キーワード

        Returns(QuerySet): キーワードがタイトルまたはコメントに含まれる投稿記事のQuerySet
        '''
        return PhotoPost.objects.filter(
            models.Q(title__icontains=keyword) |  # タイトルにキーワードを含むか
            models.Q(comment__icontains=keyword)  # コメントにキーワードを含むか
        )
